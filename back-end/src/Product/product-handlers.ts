...
import { Product, ProductFilterQuery } from "./product-model";

const createProductHandler = async (req: Request, res: Response) => {
  try {
    const productData: Omit<Product, "hasClassification"> & {
    ...
    if (!productData.name || !productData.categoryId || !productData.sellerId) {
      return res
        .status(400)
        .json({ error: "Name, category, and seller are required" });
    }
    ...
    try {
      ...
      attributeValues = parseAndEnsureArray(
        productData.attributeValues,
        "attributeValues"
      );
      classificationGroups = parseAndEnsureArray(
        productData.classificationGroups,
        "classificationGroups"
      );
      productClassifications = parseAndEnsureArray(
...
const findAllHandler = async (req: Request, res: Response) => {
  ...
  try {
    const productsFromsServer = await productService.findAllService(
      Number(page),
      Number(pageSize)
    );
    res.json({
...
const imageTester = async (req: Request, res: Response) => {
  ...
  const { id } = req.body;
  try {
    const productsFromsServer = await productService.uploadProductImages(
      files,
      id
    );
    res.json({
