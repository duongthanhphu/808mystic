...
const createProductService = async (
  ...
  try {
    const productTransaction = await prisma.$transaction(
      async (prismaTransaction) => {
        ...
        const createdProductInDB = await prismaTransaction.product.create({
        ...
        if (typeof attributeValues === "object") {
          await Promise.all(
            Object.values(attributeValues).map(async (element, index) => {
              let attributeValue =
                await prismaTransaction.attributeValue.findFirst({
                  where: {
        ...
        if (
          Array.isArray(classificationGroups) &&
          Array.isArray(productClassifications)
        ) {
          const createdGroups = await Promise.all(
            classificationGroups.map(async (group) => {
              if (!group.name) {
                throw new Error("Classification group name is required");
              }
              if (!Array.isArray(group.options) || group.options.length === 0) {
                throw new Error(
                  `Classification group "${group.name}" must have at least one valid option`
                );
              }
              return await prismaTransaction.classificationGroup.create({
                data: {
                  ...
                  options: {
                    create: group.options.map((optionName) => ({
                      name: optionName,
                    })),
                  },
                },
